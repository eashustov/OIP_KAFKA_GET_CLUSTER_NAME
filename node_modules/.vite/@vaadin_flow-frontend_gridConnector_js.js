import {
  Grid,
  ItemCache
} from "./chunk-SM4CB2QQ.js";
import "./chunk-LECCHWOK.js";
import {
  isFocusable
} from "./chunk-VVZG5RSH.js";
import "./chunk-RXMC5CCW.js";
import "./chunk-JUQHYRUK.js";
import "./chunk-YMFKJ7HD.js";
import {
  Debouncer
} from "./chunk-5FGM5SKP.js";
import "./chunk-OZDFONM3.js";
import "./chunk-2ZSVVPYF.js";
import "./chunk-HCCMTQLH.js";
import "./chunk-NG36PZNL.js";
import "./chunk-XWUB5KNI.js";
import "./chunk-4NCM436X.js";
import "./chunk-TOQEVFKZ.js";
import "./chunk-B5AFODZK.js";
import "./chunk-QISZI7VO.js";
import "./chunk-WELLTB4J.js";
import "./chunk-EHAVPSRH.js";
import "./chunk-NH7IBX4Y.js";
import "./chunk-6LFBOP6F.js";
import "./chunk-Z7TMZ43L.js";
import {
  animationFrame,
  timeOut
} from "./chunk-JQRXRBHQ.js";
  ...
  window.Vaadin.Flow.gridConnector = {
    initLazy: (grid) => tryCatchWrapper(function(grid2) {
      ...
      grid2.$connector.beforeEnsureSubCacheForScaledIndex = tryCatchWrapper(function(targetCache, scaledIndex) {
        ...
        ensureSubCacheDebouncer = Debouncer.debounce(
          ensureSubCacheDebouncer,
          animationFrame,
          () => {
      ...
      grid2.$connector.doSelection = tryCatchWrapper(function(items, userOriginated) {
        ...
        grid2.selectedItems = grid2.selectedItems.concat(items);
        items.forEach((item) => {
          if (item) {
      ...
      grid2.$connector.beforeParentRequest = tryCatchWrapper(function(firstIndex, size, parentKey) {
        ...
        parentRequestDebouncer = Debouncer.debounce(
          parentRequestDebouncer,
          timeOut.after(parentRequestDelay),
          () => {
      ...
      grid2.$connector.fetchPage = tryCatchWrapper(function(fetch, page, parentKey) {
        ...
        for (let idx = firstNeededIndex; idx <= lastNeededIndex; idx++) {
          firstNeededPage = Math.min(firstNeededPage, grid2.$connector._getPageIfSameLevel(parentKey, idx, firstNeededPage));
          lastNeededPage = Math.max(lastNeededPage, grid2.$connector._getPageIfSameLevel(parentKey, idx, lastNeededPage));
        }
      ...
      grid2.dataProvider = tryCatchWrapper(function(params, callback) {
        ...
        if (params.parentItem) {
          ...
          rootRequestDebouncer = Debouncer.debounce(
            ...
            () => {
              grid2.$connector.fetchPage((firstIndex, size) => grid2.$server.setRequestedRange(firstIndex, size), page, root);
            }
          );
        }
      });
      const sorterChangeListener = tryCatchWrapper(function(_, oldValue) {
        if (oldValue !== void 0 && !sorterDirectionsSetFromServer) {
          grid2.$server.sortersChanged(grid2._sorters.map(function(sorter) {
            return {
              path: sorter.path,
      ...
      grid2.$connector.set = tryCatchWrapper(function(index, items, parentKey) {
        ...
        for (let i = 0; i < updatedPageCount; i++) {
          ...
          grid2.$connector.doSelection(slice.filter(
            (item) => item.selected && !isSelectedOnGrid(item)
          ));
          grid2.$connector.doDeselection(slice.filter(
      ...
      const contextMenuListener = function(e) {
        ...
        grid2.$server.updateContextMenuTargetItem(key, colId);
      };
      grid2.addEventListener("vaadin-context-menu-before-open", tryCatchWrapper(function(e) {
        contextMenuListener(grid2.$contextMenuConnector.openEvent);
      }));
      ...
      grid2.addEventListener("click", tryCatchWrapper((e) => _fireClickEvent(e, "item-click")));
      grid2.addEventListener("dblclick", tryCatchWrapper((e) => _fireClickEvent(e, "item-double-click")));
      grid2.addEventListener("column-resize", tryCatchWrapper((e) => {
        const cols = grid2._getColumnsInOrder().filter((col) => !col.hidden);
        cols.forEach((col) => {
          col.dispatchEvent(new CustomEvent("column-drag-resize"));
        });
        grid2.dispatchEvent(new CustomEvent("column-drag-resize", { detail: {
          resizedColumnKey: e.detail.resizedColumn._flowId
        } }));
      }));
      grid2.addEventListener("column-reorder", tryCatchWrapper((e) => {
        const columns = grid2._columnTree.slice(0).pop().filter((c) => c._flowId).sort((b, a) => b._order - a._order).map((c) => c._flowId);
        grid2.dispatchEvent(new CustomEvent("column-reorder-all-columns", {
          detail: { columns }
        }));
      }));
      grid2.addEventListener("cell-focus", tryCatchWrapper((e) => {
        ...
        grid2.dispatchEvent(new CustomEvent("grid-cell-focus", {
          detail: {
      ...
      function _fireClickEvent(event, eventName) {
        ...
        if (eventContext.item && !isFocusable(target) && section !== "details") {
          ...
          grid2.dispatchEvent(new CustomEvent(
            eventName,
            { detail: event }
          ));
        }
      ...
      grid2.addEventListener("grid-dragstart", tryCatchWrapper((e) => {
...
//# sourceMappingURL=@vaadin_flow-frontend_gridConnector_js.js.map
