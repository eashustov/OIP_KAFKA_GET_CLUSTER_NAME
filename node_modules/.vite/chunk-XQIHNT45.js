...
// node_modules/@vaadin/time-picker/src/vaadin-time-picker-dropdown.js
var TimePickerDropdown = class extends ComboBoxDropdown {
  static get is() {
...
var TimePickerComboBox = class extends ComboBoxMixin(ThemableMixin(PolymerElement)) {
  ...
  _isClearButton(event) {
    return super._isClearButton(event) || event.type === "input" && !event.isTrusted || event.composedPath()[0].getAttribute("part") === "clear-button";
  }
  _onChange(event) {
...
var TimePicker = class extends PatternMixin(InputControlMixin(ThemableMixin(ElementMixin(PolymerElement)))) {
        ...
        value: () => {
          return {
            formatTime: (time) => {
              ...
              time.seconds !== void 0 && (timeString += `:${pad(time.seconds)}`);
              time.milliseconds !== void 0 && (timeString += `.${pad(time.milliseconds, "000")}`);
              return timeString;
            },
            parseTime: (text) => {
  ...
  __addStep(msec, step, precision) {
    ...
    var hh = Math.floor(msec / 1e3 / 60 / 60);
    msec -= hh * 1e3 * 60 * 60;
 ...
 */
//# sourceMappingURL=chunk-XQIHNT45.js.map
